...
4.  **Environment variables:**
    - `GEMINI_API_KEY` – **required**. Google AI Studio key used by `google-generativeai`.
    - `GEMINI_MODEL_NAME` – override the default `gemini-2.5-flash-lite` model.
...
This application is containerized using Docker, so it can be deployed to any cloud provider that supports containers. Here’s an example of how to deploy it using Google Cloud Run:
